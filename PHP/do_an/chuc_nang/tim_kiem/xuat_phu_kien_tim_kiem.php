<?php
    if(trim($_GET['tu_khoa'])!=""){
        ...
        $so_du_lieu=15;
        $tv_1=mysqli_query($tv,"select count(*) from phu_kien where $chuoi_tim_sql_2");
        $tv_2=mysqli_fetch_array($tv_1);
        $so_trang=ceil($tv_2[0]/$so_du_lieu);
      
        if(!isset($_GET['trang'])){$vtbd=0;}else{$vtbd=($_GET['trang']-1)*$so_du_lieu;}

        $tv_1=mysqli_query($tv,"select * from phu_kien where $chuoi_tim_sql_2 order by id desc limit $vtbd,$so_du_lieu");
        echo "<center>";
        echo "<table>";
